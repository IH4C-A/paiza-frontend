    ...
    <div className={styles.card}>
      <div className={styles.cardHeader}>
        <div className={styles.cardHeaderContent}>
          <img src={mentor.avatarSrc} alt={mentor.name} className={styles.avatar} onError={(e) => (e.currentTarget.style.display = 'none')} />
          {/* Basic fallback if image fails, can be enhanced */}
          {!mentor.avatarSrc && <div className={styles.avatarFallback}>{mentor.avatarFallback}</div>}
          
          <div className={styles.cardTitleContainer}>
            <h3 className={styles.cardTitle}>
              {mentor.name}
              <span className={`${styles.rankBadge} ${mentor.rank === 'S' ? styles.rankS : styles.rankA}`}>
                {mentor.rank}
              </span>
            </h3>
...
export default function MentorsPage() {
  ...
  const filteredMentors = useMemo(() => {
    return allMentorsData.filter(mentor => {
      const matchesTab = activeTab === 'all' || mentor.category === activeTab || mentor.specialties.some(s => s.toLowerCase().includes(activeTab));
      ...
      const matchesRank = selectedRank === 'all' || mentor.rank === selectedRank;
  ...
  return (
    <div className={styles.pageContainer}>
      {/* Header would be part of a Layout component as per your Q&A */}
      {/* For completeness, here's how the original header structure would look with CSS Modules */}
      <header className={styles.header}>
         <div className={styles.headerInner}>
           <div className={styles.logoContainer}>
             <a href="/" className={styles.logoLink}>
               <span className={styles.logoPrimaryText}>Paiza</span> Nurture
             </a>
             <nav className={styles.nav}>
               <a href="/problems" className={styles.navLink}>問題</a>
               <a href="/mentors" className={`${styles.navLink} ${styles.navLinkActive}`}>メンター</a>
               <a href="/learning" className={styles.navLink}>学習</a>
               <a href="/ranking" className={styles.navLink}>ランキング</a>
             </nav>
           </div>
        ...
        <div className={styles.container}>
          <div className={styles.titleSection}>
            <div className={styles.titleText}>
              <h1>メンター</h1>
              <p>SランクとAランクのユーザーがメンターとして質問に答えます</p>
            </div>
            <div className={styles.searchAndFilters}>
              <div className={styles.searchInputContainer}>
                <img src="/icons/search.svg" alt="Search" /> {/* Ensure this icon is in public/icons */}
                <input 
                  type="search" 
                  placeholder="メンターを検索..." 
          ...
          <div className={styles.tabsContainer}>
            ...
            <div className={styles.filtersRow}>
              ...
              >
                ...
                <option value="sql">SQL</option> {/* Added SQL based on Suzuki's data */}
